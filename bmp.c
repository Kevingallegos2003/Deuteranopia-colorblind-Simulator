...
void bmp_write(const BMP *bmp, Buffer *buf) {
    ...
    for (uint32_t y = 0; y <= (bmp->height) - 1; y++) {
        ...
        for (uint32_t x = bmp->width; x <= rounded_width - 1; x++) {
            write_uint8(buf, 0);
        }
    }
}
...
BMP *bmp_create(Buffer *buf) {
    ...
    if (bmp == NULL) {
        fprintf(stderr, "Error, bmp is NULL");
    }
    read_uint8(buf, &type1); //figure out vars u need
    read_uint8(buf, &type2);
    read_uint32(buf, &skip2);
    read_uint16(buf, &skip1);
    read_uint16(buf, &skip1);
    read_uint32(buf, &skip2);
    ...
    read_uint32(buf, &skip2);
    //assert(type1 == &'B'); //like this??
    //verify type1 == 'B' assert?????
    //verify type2 == 'M'
